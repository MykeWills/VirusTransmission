...
public class Player1WeaponShooting : MonoBehaviour {
    ...
    bool dead;
    private Vector3 BlueGuninitPos = new Vector3(0f, 0f, 0f);
    private Vector3 BlueGunfinalPos = new Vector3(0f, 0.1f, -0.3f);
    ...
    public GameObject Bullet_Emitter_Blue;
    public GameObject Bullet;
    public float Bullet_Forward_Force;

    public GameObject MuzzleBlueFlashObject;
    ...
    void Start()
    {
        ...
        var cam = GameObject.Find("MykesPlayer1Controller/Camera/Recoil/").transform;
    ...
    public void Update()
    {
        ...
        {
            PistolAmmo.color = new Color(0f, 0f, Mathf.Sin(Time.time * 10), 1.0f);
        }
      

        StartCoroutine(FindGun());
        if (Input.GetAxisRaw("Player1Fire1") == -1 && Time.time > nextFire && AmmoShot == true)
        {
            if (Ammo <= 0)
            {
                Ammo = 0;
            ...
            else
            {
                ...
                Bullet_Handler = Instantiate(Bullet, Bullet_Emitter_Blue.transform.position, Bullet_Emitter_Blue.transform.rotation) as GameObject;
        ...
        {
            MuzzleBlueFlashObject.SetActive(true);
            MuzzleBlueFlashObject.transform.Rotate(Random.Range(0, 90), Random.Range(0, 90), Random.Range(0, 90));
            muzzleFlashTimer -= Time.deltaTime;
        }
        if (muzzleFlashTimer < 0)
        {

            MuzzleBlueFlashObject.SetActive(false);
    ...
    IEnumerator FindGun()
    {
        BlueGun = GameObject.Find("MykesPlayer1Controller/Camera/Recoil/VirusGunBlue").transform;
        if (BlueGun == null)
        {
            yield return null;
        }
    ...
    public void Recoiling()
    {
        if (Recoiled)
        {
            ...
            BlueGun.transform.localPosition = Vector3.Lerp(BlueGuninitPos, BlueGunfinalPos, perc);
           
            //==========================ReturnCannonForward================================//
            if (BlueGun.transform.localPosition == BlueGunfinalPos)
            {
                ...
                BlueGun.transform.localPosition = Vector3.Lerp(BlueGunfinalPos, BlueGuninitPos, perc);
                //-------------------TurnoffLeftCannon-------------------//
                if (BlueGun.transform.localPosition == BlueGuninitPos)
                {
                    Recoiled = false;
                    currentLerpTime = 0;
                }
